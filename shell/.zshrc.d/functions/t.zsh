...
Examples:
  t
  t add dotfiles ~/dotfiles
  t add acme ~/src/acme/frontend ~/src/acme/backend
  t rm acme
  t ls
...
EOF
  ...
  case "$1" in
    add)
      if [[ $# -lt 3 ]]; then
        echo "Usage: t add <name> <path> [path...]"
        return 1
      fi
